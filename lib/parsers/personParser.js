...
function parseBiography($) {
  ...
  const container = $(linkBiography).parent('.content')[0]
  const biographyParagraph = $(container).children()[0]
  return $(biographyParagraph).text().replace(/\s+/g, ' ')
}


function parseFilms($) {
  const allHeadings = $('#filmography .content h4')
  ...
  const films = allLinks.map((i, el) => {
    ...
    return {name, id: parseInt(idInString)}
  })
  return films.get()
}
