...
function parseType($) {
  ...
  return typeText.trim().replace(/[()']/g, '') || null
}
...
function parseLength($, film) {
  ...
  if (timeWhole.indexOf('(') !== -1 && timeWhole.indexOf('x') !== -1) {
    ...
    let hours = parseInt(hoursAndMinutes[0].replace(/\D/g, ''))
    let minutes = parseInt(hoursAndMinutes[1].replace(/\D/g, ''))
